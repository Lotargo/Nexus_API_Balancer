...
Для администрирования пулов и ключей можно использовать интеграцию MCP. Сервер запускается вместе с основным процессом и предоставляет инструменты через stdio (используя скрипт-обертку).

### Конфигурация для Roo Code

Добавьте следующий блок в файл `mcp.json` (обычно находится в `~/.config/roocode/mcp.json`):
...
В настройках MCP LM Studio (раздел "Developer" -> "MCP") добавьте конфигурацию:
